 ...
 * @param divID
 * @param url 获取数据的url
 * @param type 类型（Select ， Date ，String）
 */
function getSqlCondition(param,calc,tableID,divID,url,type){
    ...
    if("7"==calc||"8"==calc){
        ...
        if(""==value){
            ...
            if("1"==calc){//等于
                ...
                condition = nparam + " like '%25"+ value+"%25'";
            }else if("12"==calc){//左模糊
                condition = nparam + " like '%25"+ value+"'";
            }else if("13"==calc){//右模糊
                condition = nparam + " like '"+ value+"%25'";
            }else{
    ...
    if(0==flag){
        ...
        window.parent.jQuery("#"+tableID).jqGrid('setGridParam',{url:url+"?condition="+condition}).trigger("reloadGrid");
    }
    return condition;
}
    ...
    if(""==value){
        ...
        if("14"==calc){
            ...
            if("1"==calc){//等于
                ...
                condition = param + " like '%25"+ value+"%25'";
            }else if("12"==calc){//左模糊
                condition = param + " like '%25"+ value+"'";
            }else if("13"==calc){//右模糊
                condition = param + " like '"+ value+"%25'";
            }else{
                var values = value.split(",");
                var temp="";
                for(var j=0;j<values.length;j++){
...
function singleShowQuery(divID1,divID2,tourl,condition){
    jQuery.ajax({
        ...
        success:function(data){
            var datas = data.split("|");
            $("#"+divID1).append(datas[0]);
            $("#"+divID2).append(datas[1]);
        },
        error: function(data) {
            alert("showQuery错误：---- "+data);
        }
